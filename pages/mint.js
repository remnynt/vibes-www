...
export default function Mint() {
  ...
  function mint (count, eth, contract, contractAddress, nonce) {
    if (contractAddress === CONTRACT_ADDRESS_VIBES) {
      count = Math.max(1, Math.min(7, count));
    }

    let isRetry = nonce !== undefined;
      ...
      if (!error) {
        ethereum.request({ method: 'eth_sendTransaction', params: [tx] })
          ...
          .catch((e) => {
            ...
            if (!isRetry && message.indexOf("nonce") >= 0) {
              web3.eth.getTransactionCount(address, "pending")
                .then((n) => mint(count, eth, contract, contractAddress, n))
                .catch(() => alertError(e));
            } else {
              alertError(e);
            }
  ...
  function mintVibes (event) {
    ...
    prepareMint(() => mint(count, 0.07, contractVibes, CONTRACT_ADDRESS_VIBES, undefined));
  };

  function mintOpenVibes (event) {
    event.preventDefault();
    prepareMint(() => mint(0, 0, contractOpenVibes, CONTRACT_ADDRESS_OPEN_VIBES, undefined));
  };
    ...
    <Layout>
      ...
      <div className={mintStyles.mint}>
        <div className={mintStyles.mint_item}>
          <h3>[genesis] vibes</h3>
          {
            totalSupplyVibes === 7777
              ...
              : mintingActiveVibes
                ? <h4enabled>minting now</h4enabled>
        ...
        <div className={mintStyles.mint_item}>
          <h3>learn more</h3>
          <p>
            read the <Link href="/about"><a>about</a></Link> page for more details and official links.
          </p>
        </div>
